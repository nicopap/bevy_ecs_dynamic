// Copy of https://github.com/rparrett/bevy_simple_text_input
// Copyright to the bevy_simple_text_input authors, licensed under MIT OR Apache-2.0
...
impl Plugin for TextInputPlugin {
    fn build(&self, app: &mut App) {
        ...
        app.add_event::<TextInputSubmitEvent>()
            .add_event::<TextInputChangeEvent>()
            .register_type::<TextInput>()
            .register_type::<TextInputInner>()
            .add_systems(Update, (create, keyboard, cursor, input_focus));
    }
}
...
fn keyboard(
    ...
    mut change_writer: EventWriter<TextInputChangeEvent>,
) {
            ...
            if let Ok(mut text) = text_query.get_mut(descendant) {
                ...
                for event in character_events.iter() {
                    if ['\u{7f}', '\u{8}'].contains(&event.char) {
                        ...
                        change_writer.send(TextInputChangeEvent {
                            entity: input_entity,
